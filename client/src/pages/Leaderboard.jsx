...
const Leaderboard = () => {
    ...
    return (
        <div className="min-h-screen premium-gradient text-black font-sans overflow-x-hidden pb-10">
            {/* Header - Operations Style */}
            <header className="px-8 pt-16 pb-8 flex items-center justify-between relative bg-white/70 backdrop-blur-xl border-b border-zinc-100/50 sticky top-0 z-[60]">
                <button 
                    onClick={() => {
                        if (isSearching) {
                    ...
                    className="p-3 bg-white text-black rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all z-10 relative"
                >
                    <BackIcon size={20} />
                </button>
                ...
                {isSearching ? (
                    <input 
                        ...
                        className="flex-1 mx-4 bg-white border border-zinc-200 px-5 py-2.5 rounded-full outline-none focus:border-emerald-500 transition-all text-sm font-semibold z-10 relative"
                    />
                ) : (
                    <h1 className="text-xl font-bold tracking-tight text-black absolute left-1/2 -translate-x-1/2">The Turf </h1>
                )}
                
                {!isSearching && (
                    <button 
                        onClick={() => setIsSearching(true)}
                        className="p-1 hover:bg-zinc-100 rounded-full text-zinc-400 transition-colors z-10 relative"
                    >
                        <SearchIcon size={26} />
                    </button>
                )}
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center mb-8 w-full drop-shadow-2xl">
                                {isRank1 && (
                                    <div className="mb-4 animate-bounce">
                                        <TrophyIcon className="text-amber-400 fill-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.5)]" size={48} />
                                    </div>
                                )}
                                ...
                                <div className={`relative mb-4 group`}>
                                    <div className={`rounded-[2.5rem] overflow-hidden border-4 bg-white shadow-2xl transition-all group-hover:rotate-6 ${
                                        ...
                                        {player.image ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-400 to-emerald-600 font-black text-3xl text-black uppercase italic">
                                                {player.name.charAt(0)}
                                            </div>
                                        )}
                                    </div>
                                    {isRank1 && (
                                        <div className="absolute -bottom-2 -right-2 bg-amber-400 text-black w-10 h-10 rounded-2xl flex items-center justify-center border-4 border-white shadow-lg">
                                            <StarIcon size={16} className="fill-zinc-900" />
                                        </div>
                                    )}
                                </div>
                                
                                <span className={`text-center font-black tracking-tighter mb-2 truncate w-full uppercase ${isRank1 ? 'text-lg text-black' : 'text-xs text-zinc-400'}`}>
                                    {player.name}
                                </span>
                                
                                <div className={`${isRank1 ? 'bg-white text-black' : 'bg-white/50 text-zinc-400'} px-4 py-1.5 rounded-2xl flex items-center gap-2 shadow-xl border border-white/20 backdrop-blur-md`}>
                            ...
                            <div className="relative w-full">
                                <div 
                                    className={`w-full rounded-[2.5rem] transition-all shadow-[0_32px_64px_-16px_rgba(0,0,0,0.3)] flex items-center justify-center font-black italic text-6xl md:text-8xl text-black/10 ${
                                        isRank1 ? 'h-64 bg-white border-t-2 border-white/10' : 
            ...
            <div className={`px-8 py-16 space-y-4 max-w-2xl mx-auto ${searchQuery ? 'min-h-[500px]' : ''}`}>
                ...
                {(searchQuery ? filteredPlayers : players.slice(3)).map((player, index) => {
                    ...
                    return (
                        ...
                        >
                            <span className="text-[18px] font-black text-zinc-200 italic w-8 tracking-tighter">#{displayRank}</span>
                            
                            <div className="w-16 h-16 rounded-[1.5rem] overflow-hidden bg-zinc-100 border-2 border-white/50 shadow-inner group-hover:rotate-6 transition-all">
                                {player.image ? (
                                    <img src={player.image} alt={player.name} className="w-full h-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-1 min-w-0">
                                <h4 className="text-xl font-black text-black tracking-tighter uppercase leading-tight mb-1">{player.name}</h4>
                ...
                {filteredPlayers.length === 0 && searchQuery && (
                    <div className="py-24 text-center text-zinc-200 flex flex-col items-center gap-4 animate-in fade-in duration-300">
                ...
                {players.length <= 3 && !searchQuery && (
                    <div className="py-24 text-center text-zinc-200 flex flex-col items-center gap-4">
                        <TrophyIcon size={56} className="opacity-10" />
                        <p className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-300">New Gladiators Joining soon...</p>
                    </div>
                )}
            </div>
        </div>
