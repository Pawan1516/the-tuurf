...
import { LayoutDashboard, Calendar, Activity, Briefcase, PieChart, LogOut, ChevronRight, Database, CheckCircle, Clock, CalendarCheck } from 'lucide-react';
import AuthContext from '../../context/AuthContext';
import { bookingsAPI, slotsAPI } from '../../api/client';
import MobileNav from '../../components/MobileNav';

const WorkerBookedSlots = () => {
    ...
    useEffect(() => {
        const fetchBookings = async () => {
            try {
                setLoading(true);
                // Worker can only see all bookings on their turf or assigned slots via getMySlots()
                const response = await bookingsAPI.getMySlots();
                const allBookings = response.data.bookings || [];
                // Filter for only confirmed/hold
                const booked = allBookings.filter(b => ['confirmed', 'hold'].includes(b.bookingStatus));
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {bookings.filter(b => b.slot?.date ? new Date(b.slot.date).toISOString().split('T')[0] === filterDate : false).length === 0 ? (
                                ...
                                >
                                    ...
                                    <div className="mt-6 p-4 bg-gray-50 rounded-2xl flex items-center gap-4 border border-gray-100 border-dashed">
