...
import { LogOut, FileText, Database, ShieldCheck, ChevronRight, TrendingUp, Cpu, LayoutDashboard, Calendar, Activity, Briefcase, PieChart } from 'lucide-react';
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="bg-white/80 backdrop-blur-md px-10 h-24 flex items-center justify-between sticky top-0 z-40 border-b border-gray-100">
          <div className="flex flex-col">
            <h2 className="text-2xl font-black text-gray-900 tracking-tighter uppercase leading-none">Report Generation Console</h2>
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Download the report</p>
          </div>
        </header>
        ...
        <div className="p-10 space-y-10">
          {error && (
            <div className="bg-red-50 border border-red-100 p-6 rounded-[2rem] text-red-600 text-xs font-black uppercase tracking-tight flex items-center gap-4">
              <ShieldCheck className="rotate-180" /> {error}
            </div>
          )}
          ...
          <div className="grid lg:grid-cols-2 gap-10">
            ...
            <div className="bg-emerald-900 rounded-[3rem] p-12 text-white shadow-2xl shadow-emerald-900/20 flex flex-col justify-between group overflow-hidden relative">
              ...
              <div className="relative z-10 space-y-8">
                <div className="space-y-2">
                  <p className="text-[10px] font-black text-emerald-400/60 uppercase tracking-[0.3em]">report generation</p>
                  <h3 className="text-4xl font-black tracking-tighter uppercase leading-none">select <span className="text-emerald-400 italic">and download</span></h3>
                </div>
                <p className="text-emerald-100/40 text-sm font-medium leading-relaxed max-w-sm">
                  Generates a cryptographically verified PDF containing revenue aggregates, operative performance, and granular booking metadata.
                </p>
                ...
                <div className="space-y-6 pt-10">
                  <div className="flex gap-2">
                    {['all', 'daily', 'weekly', 'monthly'].map(p => (
                      <button
                        key={p}
                        onClick={() => setPeriod(p)}
                        className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${period === p
                  ...
                  <div className="flex gap-4 pt-10">
                    <button
                      ...
                      className="flex-1 bg-white text-emerald-900 py-6 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-emerald-400 hover:text-black transition-all shadow-xl shadow-black/20"
                    >
                      <TrendingUp size={18} className="rotate-45" />
                      {downloading ? 'Compiling...' : 'PDF Report'}
                    </button>
                    <button
                      onClick={() => handleDownloadReport('csv')}
                      disabled={downloading}
                      className="flex-1 bg-white/10 text-white py-6 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-white/20 transition-all"
                    >
            ...
            <div className="space-y-8">
              <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-xl shadow-emerald-900/5">
                <h4 className="text-xs font-black text-gray-900 uppercase tracking-[0.2em] mb-8 flex items-center gap-3">
                  <TrendingUp size={18} className="text-emerald-600" /> the turf
                </h4>
                <div className="space-y-4">
                  {[
                    { label: 'Revenue Matrix', desc: 'Aggregated financial intake by unit' },
                    { label: 'Operative Logs', desc: 'Ground personnel efficiency metrics' },
                    { label: 'Slot Density', desc: 'Infrastructure utilization heatmaps' },
                    { label: 'Traffic Audit', desc: 'Complete booking lifecycle tracking' }
                  ].map((item, i) => (
                    <div key={i} className="flex items-start gap-4 p-4 rounded-2xl hover:bg-emerald-50/50 transition-colors group">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase leading-none">{item.desc}</p>
                      </div>
                    </div>
                  ))}
                </div>
