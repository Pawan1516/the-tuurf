...
import { Calendar, Clock, MapPin, ChevronRight, Activity, LogOut, LayoutDashboard, Database, CheckCircle, XCircle, Clock4, Zap } from 'lucide-react';
...
const UserDashboard = () => {
    ...
    const getStatusInfo = (status) => {
        switch (status) {
            ...
            case 'hold': return { icon: <Clock4 className="text-yellow-500" size={16} />, label: 'ON HOLD', color: 'text-yellow-600', bg: 'bg-yellow-50' };
            ...
            <aside className="w-full md:w-80 bg-white border-r border-gray-100 flex flex-col sticky top-0 md:h-screen z-50">
                ...
                <div className="flex-1 p-6 space-y-2">
                    <div className="p-6 bg-emerald-50 rounded-[2rem] border border-emerald-100 mb-8">
                        ...
                        <h3 className="text-lg font-black text-emerald-900 uppercase truncate">{user?.name}</h3>
                        <p className="text-[10px] font-bold text-emerald-600/60 truncate">{user?.email}</p>
                    </div>

                    <button className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl bg-emerald-600 text-white shadow-xl shadow-emerald-200 transition-all">
                ...
                <div className="p-6 md:p-10 space-y-12">
                    ...
                    <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-xl shadow-emerald-900/[0.02]">
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <button onClick={() => navigate('/')} className="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline">View Full Schedule</button>
                        </div>
                                ...
                                todaySlots.sort((a, b) => a.startTime.localeCompare(b.startTime)).map((slot) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={`flex-shrink-0 w-32 p-6 rounded-3xl border-2 transition-all cursor-pointer ${slot.status === 'free'
                                        ...
                                        >
                                            <p className="text-xs font-black text-gray-900 mb-1">{slot.startTime}</p>
                                            <p className={`text-[10px] font-black uppercase tracking-widest ${slot.status === 'free' ? 'text-emerald-600' : isBooked ? 'text-red-500' : 'text-yellow-600'}`}>
                    ...
                    <div className="space-y-6">
                        ...
                        {bookings.length === 0 ? (
                            <div className="bg-white rounded-[3rem] p-20 text-center border border-gray-100 shadow-xl shadow-emerald-900/[0.02]">
                                <div className="bg-emerald-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 text-emerald-600">
                                    <Calendar size={40} />
                                </div>
                                <h3 className="text-2xl font-black text-gray-900 mb-2 uppercase">No Bookings Found</h3>
                                <p className="text-gray-400 font-medium mb-8">You haven't reserved any slots yet. Ready to play?</p>
                                <button onClick={() => navigate('/')} className="bg-emerald-600 text-white px-10 py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200">
                                    Start Reservation
                                </button>
                            </div>
                        ) : (
                            <div className="grid gap-6">
                                {bookings.map((booking) => {
                                    ...
                                    return (
                                        <div key={booking._id} className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-gray-100 shadow-xl shadow-emerald-900/[0.02] flex flex-col md:flex-row items-center gap-8 group hover:border-emerald-200 transition-all">
                                            <div className="bg-emerald-50 w-20 h-20 rounded-3xl flex flex-col items-center justify-center text-emerald-600 shrink-0 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                                            ...
                                            <div className="flex-1 space-y-4 text-center md:text-left">
                                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                                    <h4 className="text-xl font-black text-gray-900 tracking-tight uppercase">#{booking._id.slice(-6)}</h4>
                                                    <div className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2 border ${status.bg} ${status.color} border-current opacity-70`}>
                                                        {status.icon}
                                                        {status.label}
                                                    </div>
                                                </div>

                                                <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-8 gap-y-2 opacity-60">
                                            ...
                                            <div className="text-center md:text-right shrink-0">
                                                <p className="text-[10px] font-black text-gray-300 uppercase tracking_widest mb-1">Transaction Value</p>
                                                <p className="text-2xl font-black text-emerald-600 tracking-tighter">₹{booking.amount}</p>
                                                <p className={`text-[9px] font-black uppercase tracking-widest mt-1 ${booking.paymentStatus === 'verified' ? 'text-emerald-400' : 'text-yellow-500'}`}>
